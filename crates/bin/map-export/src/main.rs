...
//! `map-export --seed 42 --size 512 --layer terrain --out maps/terrain-42.bmp`
...
fn main() {
    ...
    while i < args.len() {
        ...
        match (args[i].as_str(), value) {
            ...
            ("--provinces", Some(v)) => province_count = v.parse().unwrap_or_else(|_| usage()),
    ...
    let color: fn(&sim_server::Genesis, usize) -> palette::Rgb = match layer.as_str() {
        ...
        "provinces" => palette::provinces_layer,
        _ => usage(),
    };

    let genesis = sim_server::genesis(WorldSeed(seed), size, province_count);
    ...
    let habitable = genesis.provinces.iter().filter(|p| p.habitable).count();
    println!(
        "{} · seed {seed} · {size}² · land {} · {} provinces ({habitable} habitable) · {} flora species",
        path.display(),
        genesis.fields.land_cells(),
        genesis.provinces.len(),
...
fn usage() -> ! {
    eprintln!(
        "usage: map-export [--seed N] [--size N] [--provinces N] \
         [--layer terrain|height|flora|provinces] [--out path.bmp]"
    );
    std::process::exit(2)
}
