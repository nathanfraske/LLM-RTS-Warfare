...
**Ships now (U0 + first fire):** mineral palette, event history, per-tile columns (bedrock, veins, faults, caves, vents), geothermal genesis warmth, live eruptions with lava runs and their full surface consequences, inspector lines, events in feed and chronicle. **Queued:** earthquakes on the faults; underground local maps; prospecting via the knowledge layer; extraction verbs and grammar binding (U2); hydrothermal vein growth along faults; obsidian-grade knappable stone as an early-tech material bid.
